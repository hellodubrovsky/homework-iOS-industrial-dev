...
final class AppManager {
    ...
    private init() {
        ...
        let favoriteItemTabBar = factory.makeTabBarItem(title: "Favorite", image: UIImage(systemName: "star.circle.fill")!)
        ...
        let favoriteNavigationController = factory.makeNavigatioController(viewController: favoritePostsViewController, taBarItem: favoriteItemTabBar)
        
        // Create main tab bar controller
        let rootCoordinator = MainCoordinatorImplementation()
        let rootTabBarViewController = MainTabBarController(coordinator: rootCoordinator, viewControllers: [profileNavigationController, feedNavigationController, mediaNavigationController, favoriteNavigationController])
